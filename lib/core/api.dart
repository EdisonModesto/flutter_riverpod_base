...
class API {
  ...
  FutureEither<Response> getRequest({required String url, bool requireAuth = true}) async {
    final Map<String, String> requestHeaders = {"Content-Type": "application/json", "Authorization": "token=$_authToken"};
    if (requireAuth) {
    ...
    try {
      final response = await _dio.get(url, options: Options(headers: requestHeaders));
  ...
  FutureEither<Response> postRequest({required String url, dynamic body, bool requireAuth = true}) async {
    ...
    try {
      final response = await _dio.post(url, data: body, options: Options(headers: requestHeaders));
  ...
  FutureEither<Response> putRequest({required String url, dynamic body, bool requireAuth = true}) async {
    ...
    try {
      final response = await _dio.put(url, data: body, options: Options(headers: requestHeaders));
  ...
  FutureEither<Response> deleteRequest({required String url, dynamic body, bool requireAuth = true}) async {
    ...
    try {
      final response = await _dio.delete(url, data: body, options: Options(headers: requestHeaders));
